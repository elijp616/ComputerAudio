...
void setup() {
  ...
  ps.addListener(f);
  frequencyGlide = new Glide(ac, 100, 10);
  wp = new WavePlayer(ac, frequencyGlide, Buffer.SINE);
  wpGlide = new Glide(ac, 1, 1000);
  wpGain = new Gain(ac, 1, wpGlide);
  wpGain.addInput(wp);

  SpectralDifference sd = new SpectralDifference(ac.getSampleRate());
  ps.addListener(sd);
  beatFinder = new PeakDetector();
  sd.addListener(beatFinder);
  beatFinder.setThreshold(.2f);
  beatFinder.setAlpha(.9f);
  beatFinder.addMessageListener(
    new Bead() {
...
void draw()
{
  ...
  // Find strongest frequency and comput meanFrequency - see 9.3
    ...
    float inputFreq = f.getFeatures();
    if (inputFreq < 3000) {
      meanFrequency = (.4 * inputFreq) + (.6 * meanFrequency);
